...
<template>
  <v-app class="ma-5">
      ...
      <div class="px-4">
        ...
        >
          ...
          <template v-slot:item.controls-order="props">
            <v-btn
              v-if="props.index < clients.length - 1"
              class="mr-2"
              size="small"
              variant="elevated"
              color="info"
              icon="mdi-arrow-down-bold"
              @click="reorder(props.item, 'down')"
            />
            <v-btn
              v-if="props.index > 0"
              size="small"
    ...
    <v-dialog v-model="dialogRemove" persistent width="auto">
      <v-card>
        <template #text> ¿Está seguro de eliminar el registro? </template>
        <template #actions>
          <v-spacer></v-spacer>
          <v-btn variant="elevated" color="primary" @click="removeClient()"> Confirmar </v-btn>
          <v-btn variant="elevated" color="red" @click="dialogRemove = false"> Cancelar </v-btn>
        </template>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="dialogCreate" persistent width="auto">
      <CreateClient
        ...
        @cancel-edit="dialogCreate = false"
